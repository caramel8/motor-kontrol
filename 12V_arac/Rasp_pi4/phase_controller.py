...
class PhaseLockPD(Node):
    def __init__(self):
        ...
        # Mutlak hedefi Motor 4 kullanır (isim target1 ama Motor4 içindir)
        ...
        self.min_run    = float(self.declare_parameter('min_run', 80.0).value)
        self.log        = bool(self.declare_parameter('log', True).value)

        # Faz offset (counts) — “geride” için ref = ang4 - offset
    ...
    def enc_cb(self, msg: Int32MultiArray):
        ...
        #yazdırmak için 
        e1_deg=e1/self.cpr*360
        e2_deg=e2/self.cpr*360
        e3_deg=e3/self.cpr*360


        if self.log:
            self.get_logger().info(
                f"u=[{u1},{u2},{u3},{u4}] | ang=[{ang1},{ang2},{ang3},{ang4}] | ref1={ref1} ref23={ref23} | e=[{e1},{e2},{e3}] err_abs={err_abs} angle_diff_in_degree={e1_deg},{e2_deg} {e3_deg}"
            )
